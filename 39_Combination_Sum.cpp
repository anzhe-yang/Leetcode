...
void combinationSum(vector<int>& candidates, int target, vector<int>& temp, int start, vector<vector<int>>& res)
{
    if (target == 0)
    {
        res.push_back(temp);
    ...
    for (int i = start; i != candidates.size() && target >= candidates[i]; i++)
    {
        ...
        combinationSum(candidates, target-candidates[i], temp, i, res);
        temp.pop_back();
    }
}
...
vector< vector<int> > combinationSum(vector<int>& candidates, int target)
{
    ...
    */
    ...
    vector<int> temp;
    combinationSum(candidates, target, temp, 0, res);
    return res;
}
